...
export default function GalleryPage() {
  ...
  const nextImage = () => {
    if (activeAlbumIdx === null) return;
    const album = albums[activeAlbumIdx];
    setActiveImageIdx((prev) => (prev + 1) % album.images.length);
  };

  const prevImage = () => {
    if (activeAlbumIdx === null) return;
    const album = albums[activeAlbumIdx];
    setActiveImageIdx((prev) => (prev - 1 + album.images.length) % album.images.length);
  };
          ...
          >
            ...
            <div className="flex justify-between items-center w-full z-10">
              <div className="text-white">
                <h2 className="text-lg md:text-2xl font-black tracking-tight">{albums[activeAlbumIdx].title}</h2>
                <p className="text-xs md:text-sm text-slate-400 font-medium mt-1">
                  Photo {activeImageIdx + 1} of {albums[activeAlbumIdx].images.length}
                </p>
              </div>
              <button
                onClick={closeAlbum}
            ...
            <div className="relative flex items-center justify-center w-full h-[60vh] md:h-[70vh]">
              ...
              <div className="relative w-full h-full max-w-5xl mx-4 flex items-center justify-center">
                ...
                >
                  <Image
                    src={`/gallery/${albums[activeAlbumIdx].images[activeImageIdx]}`}
                    alt={`${albums[activeAlbumIdx].title} - Photo ${activeImageIdx + 1}`}
                    fill
            ...
            <div className="w-full flex justify-center items-center gap-3 overflow-x-auto py-4 px-2 max-w-4xl mx-auto scrollbar-thin scrollbar-thumb-white/20">
              {albums[activeAlbumIdx].images.map((img, i) => (
                <button
                  key={i}
